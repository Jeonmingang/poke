...
public class PixelmonSpecUtil {
    ...
    public static Result extractAndRemovePartyPokemonWithNBT(Player p, int slot1to6) {
        ...
        try {
            ...
            r.specString = buildSpecWithoutMoves(r);      // ✅ 폴백 스펙은 moves 제외
    ...
    private static void fillInfoFromPokemon(Object pk, Result r) throws Exception {
        // 종
        try { Object sp = pk.getClass().getMethod("getSpecies").invoke(pk);
              String n = asText(sp,"getLocalizedName","getName");
              r.species = stripPkg(n!=null?n:String.valueOf(sp)); } catch(Throwable ignore){}
        // 닉네임/레벨/성별
        ...
        try { String g = String.valueOf(pk.getClass().getMethod("getGender").invoke(pk));
              r.gender = "MALE".equalsIgnoreCase(g)?"수컷": "FEMALE".equalsIgnoreCase(g)?"암컷": "무성"; } catch(Throwable ignore){}
        // 성격/특성
        ...
        try { Object fo = pk.getClass().getMethod("getForm").invoke(pk);
              ...
              r.form = f; } catch(Throwable ignore){}
    ...
    private static String fromComponentOrString(Object v){
        try { Class<?> ic = Class.forName("net.minecraft.util.text.ITextComponent");
              if (ic.isInstance(v)) {
    ...
    private static String readStats(Object pk, boolean iv){
        try {
            ...
            int[] arr = new int[6];
            try { Object a = stats.getClass().getMethod("getArray").invoke(stats);
            ...
            try { Method get = stats.getClass().getMethod("get", int.class);
                  for(int i=0;i<6;i++){ Object v = get.invoke(stats,i); if(v instanceof Number) arr[i]=((Number)v).intValue(); }
                  return join6(arr); } catch (Throwable ignore){}
        } catch (Throwable ignore){}
